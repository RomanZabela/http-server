...
export class StoreService implements IStoreService { 
    ...
    public updateStore(storeType: Stores, userId: number): Promise<Stores> {
        return new Promise<Stores>((resolve, reject) => {
            ...
            SQLHelper.executeQueryUpdate(this._errorService, StoreQueries.updateStoreCapacity, storeType.storeCapacity, storeType.storeName, storeType.id, storeType.storeActive)
            .then(() => {
                resolve(storeType);
            })
            .catch((error: systemError) => {
